...
//-----------------------------------------------------------
class BDFuel_SpillFire extends Projectile;
...
simulated function CatchFire()
{
    ...
    Flames=Spawn(class'BDFuelFlame');
...
simulated State OnFire
{
    simulated function Spread()
    {
      ...
      foreach RadiusActors(class'BDFuel_SpillFire', SF, 100)
      {
          if (!SF.bOnFire)
           {
            SF.CatchFire();
    ...
    simulated function FindPlayers()
    {
     ...
     foreach RadiusActors(class'xPawn', P, 50)
     {
      AttachFlames(P);
     }
    }
    ...
    simulated function AttachFlames(xPawn P)
    {
      ...
      foreach RadiusActors(class'BDFlameThrower_FireAttachment', OldAttach, 80)
      {
       if (OldAttach.Base == P)
       {
